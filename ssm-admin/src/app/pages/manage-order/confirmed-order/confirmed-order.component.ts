...
export class ConfirmedOrderComponent implements OnInit {
  ...
  ngOnInit() {
    this._orderService.countConfirmedOrder()
    .subscribe(
      res=>{
        this.totalPages=Math.ceil(res[0].confirmedOrder/this.limit);
        this.lastPage=this.totalPages;
        
        if(this.totalPages>0){
          this.getConfirmedOrder(1);
          this.setPagination();
        }
      }
    );
  ...
  getConfirmedOrder(pageno){
    this._orderService.getConfirmedOrder((pageno*10)-10)
    .subscribe(
      res=>{
        //@ts-ignore
        this.orders=res;
  ...
  dispatchOrder(order){
    this._orderService.dispatchOrder(order.item_id)
    .subscribe(
      res=>{
        //@ts-ignore
        if(res.success==true){
